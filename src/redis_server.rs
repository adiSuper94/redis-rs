    ...
    pub fn execute(&mut self, command: &Command) -> Vec<String> {
        match &command {
            ...
            Command::Psync(_repl_id, _offset) => {
                ...
                let base64_ecoded_rdb = "UkVESVMwMDEx+glyZWRpcy12ZXIFNy4yLjD6CnJlZGlzLWJpdHPAQPoFY3RpbWXCbQi8ZfoIdXNlZC1tZW3CsMQQAPoIYW9mLWJhc2XAAP/wbjv+wP9aog==";
                match BASE64_STANDARD.decode(base64_ecoded_rdb) {
                    Ok(bytes) => {
                        let empty_rdb = String::from_utf8(bytes)
                            .expect("Error while parsing u8 arr into stirng");
                        return [
                    ...
                    Err(e) => {
                        println!("error occured while decoding base64 string : {}", e);
                        [].to_vec()
                    }
                }
            }
