...
pub struct Redis {
    ...
    master_stream: Arc<Mutex<Option<TcpStream>>>,
    replica_stream: Arc<Mutex<Option<TcpStream>>>,
}
...
impl Redis {
    pub async fn new(cli_args: RedisCliArgs) -> Self {
        let mut instance = Redis {
            ...
            master_stream: Arc::new(Mutex::new(None)),
            replica_stream: Arc::new(Mutex::new(None)),
        };
    ...
    pub fn clone(&self) -> Self {
        let clone = Redis {
            ...
            master_stream: Arc::clone(&self.master_stream),
            replica_stream: Arc::clone(&self.replica_stream),
        };
        clone
    }
    ...
    async fn handshake_with_master(&mut self) {
        ...
        write(&stream, msg.as_bytes()).await;
        self.master_stream = Arc::new(Mutex::new(Some(stream)));
    }

    pub async fn execute(&mut self, command: Command, stream: &TcpStream) {
        ...
        if replicate {
            println!("propagting to replica");
            self.replicate(command).await;
        }
    }

    async fn replicate(&self, cmd: Command) {
        println!("propagting to replica 1");
        let stream_clone = Arc::clone(&self.replica_stream);
        println!("propagting to replica 2");
        tokio::spawn(async move {
            let guard = stream_clone.lock().await;
            match *guard {
                Some(ref stream) => {
                    println!("propagting to replica 3");
